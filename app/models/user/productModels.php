...
function getAllPostBySearch($seacrh)
{
    $sql = "SELECT * FROM posts 
        ...
        ON posts.category_id = post_categories.id_post_category 
        INNER JOIN users 
        ON posts.user_id = users.id_user 
        WHERE isAccepted = 0 AND post_categories.delCategory=0 AND users.isActive=0  AND (postName like '%$seacrh%' OR users.fullName like '%$seacrh%')";
    return pdo_query($sql);
}
